import { ArrowUp, Boxes, Braces, Brain, Code, Cpu, Database, FileCode, GitBranch, GitCommit, Globe, Layers, Lock, Puzzle, RefreshCw, Server, ShieldCheck, Sliders, Terminal, Zap } from 'lucide-react';
...
export const LESSONS_V2: LessonV2[] = [
  ...
  {
    schemaVersion: 2, id: 'lesson-v2-ia-prompts', moduleId: 'mod-9-v2', phaseId: 'fase-3', order: 25,
    title: 'V2.25 Prompts estructurados: pedir una respuesta útil y verificable', category: 'IA',
    summary: 'Define rol, contexto, tarea, restricciones y formato de salida en lugar de pedir respuestas vagas.',
    prerequisiteLessonIds: ['lesson-v2-testing-mocks-rtl'], estimatedMinutes: 40, xpReward: 90, icon: Cpu, color: 'from-pink-500 to-rose-600', week: 'Piloto v2', hours: 1,
    learningObjectives: ['Distinguir una petición vaga de un prompt con contexto y criterio verificable.', 'Separar instrucciones, datos y formato de salida.', 'Reconocer que una respuesta de IA debe revisarse antes de usarla.'],
    concept: { explanationMarkdown: 'Un prompt útil define quién debe responder, qué contexto puede usar, qué tarea concreta debe resolver, qué restricciones respetar y en qué formato devolver el resultado. Pedir "haz mi código" deja decisiones críticas sin especificar; pedir una función con entradas, salidas, casos límite y formato de respuesta permite revisar si cumplió.', whyItMattersMarkdown: 'Un LLM genera texto probable, no evidencia de que el resultado sea correcto. Un prompt estructurado reduce ambigüedad y hace más fácil comprobar la respuesta con pruebas, documentación o tu propio criterio.', realWorldContextMarkdown: 'Para SASE, no pedirías "haz un reporte". Definirías datos permitidos, grupo, periodo, campos necesarios, tono y que no invente calificaciones. Después revisarías el resultado antes de compartirlo.', narrationText: 'Un prompt estructurado incluye contexto, tarea, restricciones y formato. La IA propone una respuesta; tú verificas que sea correcta y segura antes de usarla.' },
    examples: [{ id: 'ejemplo-vago', title: 'De petición vaga a instrucción verificable', code: 'Vago: "Haz un reporte de alumnos"\n\nEstructurado:\nRol: asistente de análisis escolar.\nContexto: usa solo el arreglo de alumnos proporcionado.\nTarea: resume asistencia por grupo.\nRestricción: no inventes datos ni nombres.\nSalida: tabla Markdown con grupo, total y porcentaje.', explanationMarkdown: 'Cada parte limita una fuente de ambigüedad y hace observable si la respuesta cumplió.' }],
    guidedPractice: [
      { id: 'guiado-1', order: 1, promptMarkdown: '¿Qué parte del prompt impide que la IA invente datos fuera del material entregado?', options: ['Una restricción explícita', 'El saludo inicial', 'El nombre del modelo', 'Una pregunta abierta'], correctAnswer: 'Una restricción explícita', hints: ['Una restricción dice qué no debe hacer el modelo.'], explanationMarkdown: 'Indicar "usa solo los datos proporcionados" limita el alcance y facilita detectar incumplimientos.' },
      { id: 'guiado-2', order: 2, promptMarkdown: '¿Qué hace más verificable una respuesta?', options: ['Pedir un formato y criterios concretos de salida', 'Pedir que sea creativa', 'Usar más signos de exclamación', 'Ocultar el contexto'], correctAnswer: 'Pedir un formato y criterios concretos de salida', hints: ['Piensa qué permite comparar el resultado contra una expectativa.'], explanationMarkdown: 'Un formato definido permite revisar campos, estructura y omisiones de forma concreta.' },
      { id: 'guiado-3', order: 3, promptMarkdown: 'Una IA entrega código convincente. ¿Qué sigue antes de usarlo?', options: ['Revisarlo y probarlo con casos reales', 'Copiarlo directo a producción', 'Asumir que es correcto por ser detallado', 'Eliminar las pruebas'], correctAnswer: 'Revisarlo y probarlo con casos reales', hints: ['La fluidez del texto no prueba corrección.'], explanationMarkdown: 'El resultado es una propuesta; pruebas y revisión humana siguen siendo necesarias.' },
    ],
    challenge: { id: 'reto-prompts', promptMarkdown: 'Completa `construirPrompt` para organizar rol, contexto, tarea y restricción en un texto verificable.', starterCode: 'function construirPrompt(rol, contexto, tarea, restriccion) {\n  // Retorna cuatro líneas: "Rol: ...", "Contexto: ...", "Tarea: ...", "Restriccion: ..."\n  return "";\n}\n\nconsole.log(construirPrompt("analista escolar", "grupo 3A", "resumir asistencia", "no inventar datos"));', language: 'javascript', timeoutMs: 2000, checks: [ { id: 'check-rol', type: 'stdoutIncludes', label: 'Rol explícito', value: 'Rol: analista escolar', failureMessage: 'El prompt debe declarar el rol.' }, { id: 'check-contexto', type: 'stdoutIncludes', label: 'Contexto explícito', value: 'Contexto: grupo 3A', failureMessage: 'El prompt debe declarar el contexto.' }, { id: 'check-restriccion', type: 'stdoutIncludes', label: 'Restricción explícita', value: 'Restriccion: no inventar datos', failureMessage: 'El prompt debe incluir la restricción de no inventar datos.' } ], hints: ['Usa un template literal con saltos de línea.', 'Conserva exactamente las etiquetas Rol, Contexto, Tarea y Restriccion.'], expectedEvidenceMarkdown: 'La consola debe mostrar las cuatro secciones del prompt, incluyendo la restricción.' },
    reflectionPromptMarkdown: '¿Qué dato o restricción nunca dejarías fuera de un prompt que produzca un reporte para tu grupo?', masteryCriteria: ['Estructuro un prompt con contexto, tarea, restricciones y salida.', 'Diferencio una propuesta de IA de una respuesta verificada.', 'Escribí una función que produce una instrucción revisable.']
  },
  {
    schemaVersion: 2, id: 'lesson-v2-ia-rag', moduleId: 'mod-9-v2', phaseId: 'fase-3', order: 26,
    title: 'V2.26 RAG: responder con fuentes recuperadas, no con memoria inventada', category: 'IA', summary: 'Entiende cómo Retrieval Augmented Generation aporta documentos relevantes al modelo antes de responder.', prerequisiteLessonIds: ['lesson-v2-ia-prompts'], estimatedMinutes: 45, xpReward: 100, icon: Cpu, color: 'from-pink-500 to-rose-600', week: 'Piloto v2', hours: 1,
    learningObjectives: ['Explicar las etapas recuperar, aportar contexto y generar.', 'Distinguir una fuente recuperada de una afirmación sin respaldo.', 'Simular una búsqueda simple que selecciona contexto relevante.'],
    concept: { explanationMarkdown: 'RAG significa Retrieval Augmented Generation. Antes de pedir una respuesta, el sistema recupera fragmentos relevantes de una fuente controlada, los agrega al prompt como contexto y entonces el modelo genera una respuesta basada en esos fragmentos. RAG no vuelve verdadero todo lo que dice el modelo: permite citar, revisar y actualizar la fuente usada.', whyItMattersMarkdown: 'Un LLM no conoce automáticamente documentos internos ni cambios recientes. Darle fuentes seleccionadas reduce respuestas inventadas y permite que una persona compruebe de dónde salió una afirmación.', realWorldContextMarkdown: 'Un asistente para SASE podría recuperar solo el reglamento escolar o una guía autorizada antes de responder una duda docente. No debería consultar expedientes completos ni mandar datos personales innecesarios al modelo.', narrationText: 'RAG primero recupera fuentes relevantes, luego las entrega como contexto y finalmente genera una respuesta. La fuente sigue siendo revisable.' },
    examples: [{ id: 'ejemplo-rag', title: 'Las tres etapas de RAG', code: 'Pregunta: "¿Cuál es la asistencia mínima?"\n1. Recuperar: buscar en reglamento el fragmento sobre asistencia.\n2. Aportar contexto: enviar ese fragmento junto con la pregunta.\n3. Generar: responder citando el fragmento recuperado.', explanationMarkdown: 'La respuesta debe basarse en el fragmento recuperado, no en una regla inventada por el modelo.' }],
    guidedPractice: [
      { id: 'guiado-1', order: 1, promptMarkdown: '¿Qué etapa busca fragmentos relevantes antes de llamar al modelo?', options: ['Recuperación', 'Generación', 'Despliegue', 'Entrenamiento'], correctAnswer: 'Recuperación', hints: ['Es la R de RAG.'], explanationMarkdown: 'La recuperación selecciona documentos o fragmentos que aportarán evidencia al prompt.' },
      { id: 'guiado-2', order: 2, promptMarkdown: '¿Qué ventaja aporta incluir la fuente recuperada?', options: ['Permite revisar y citar de dónde salió la respuesta', 'Garantiza que nunca habrá errores', 'Elimina toda necesidad de privacidad', 'Convierte la IA en una base de datos'], correctAnswer: 'Permite revisar y citar de dónde salió la respuesta', hints: ['La fuente no elimina la revisión, pero hace posible rastrearla.'], explanationMarkdown: 'RAG mejora trazabilidad; la persona puede contrastar respuesta y fuente.' },
      { id: 'guiado-3', order: 3, promptMarkdown: '¿Qué contexto NO deberías recuperar para responder una duda general sobre reglamento?', options: ['Expedientes completos con datos personales no necesarios', 'El fragmento autorizado del reglamento', 'La pregunta concreta del docente', 'La fecha de vigencia de la regla'], correctAnswer: 'Expedientes completos con datos personales no necesarios', hints: ['Recuperar solo lo mínimo reduce exposición de datos.'], explanationMarkdown: 'RAG debe minimizar contexto: datos personales ajenos a la pregunta no deben entrar al prompt.' },
    ],
    challenge: { id: 'reto-rag', promptMarkdown: 'Completa `recuperarContexto` para devolver solo los documentos cuyo texto incluya una palabra clave, simulando la recuperación previa a un prompt.', starterCode: 'function recuperarContexto(documentos, palabraClave) {\n  // Devuelve los textos que incluyan palabraClave sin distinguir mayúsculas.\n  return [];\n}\n\nconst documentos = ["Reglamento: asistencia mínima 80%.", "Guía: las calificaciones van de 0 a 10.", "Aviso: reunión docente el viernes."];\nconsole.log(recuperarContexto(documentos, "asistencia").join(" | "));', language: 'javascript', timeoutMs: 2000, checks: [{ id: 'check-contexto', type: 'stdoutIncludes', label: 'Recupera el reglamento relevante', value: 'Reglamento: asistencia mínima 80%.', failureMessage: 'Debe recuperarse el documento que contiene asistencia.' }, { id: 'check-no-ajeno', type: 'stdoutEquals', label: 'No agrega documentos no relevantes', value: 'Reglamento: asistencia mínima 80%.', failureMessage: 'La salida debe contener solo el documento recuperado, no los demás.' }], hints: ['Convierte texto y palabraClave a minúsculas.', 'Usa documentos.filter(...).'], expectedEvidenceMarkdown: 'La consola debe mostrar solo el fragmento de reglamento sobre asistencia.' },
    reflectionPromptMarkdown: '¿Qué fuente autorizada recuperarías para responder una duda docente y qué dato sensible excluirías del contexto?', masteryCriteria: ['Explico recuperar, aportar contexto y generar.', 'Reconozco que una fuente recuperada debe poder revisarse.', 'Minimizo datos en el contexto.', 'Simulé una recuperación de documentos relevantes.']
  },
  {
    schemaVersion: 2, id: 'lesson-v2-ia-api-segura', moduleId: 'mod-9-v2', phaseId: 'fase-3', order: 27,
    title: 'V2.27 APIs LLM: integrar sin exponer claves ni aceptar salidas ciegamente', category: 'IA', summary: 'Diseña el contrato de una llamada a LLM con contexto mínimo, respuesta estructurada y manejo de errores.', prerequisiteLessonIds: ['lesson-v2-ia-rag'], estimatedMinutes: 50, xpReward: 110, icon: Cpu, color: 'from-pink-500 to-rose-600', week: 'Piloto v2', hours: 1,
    learningObjectives: ['Identificar por qué una clave de API nunca vive en el navegador.', 'Definir una respuesta estructurada y validar su forma.', 'Manejar éxito y error sin depender de un proveedor real.'],
    concept: { explanationMarkdown: 'Una API de LLM debe llamarse desde un servidor o función protegida, nunca desde el navegador con una clave secreta. El cliente envía una solicitud mínima a un endpoint propio; el servidor añade la clave, limita contexto y valida la respuesta antes de devolver solo los campos permitidos. Una salida fluida puede estar incompleta o ser incorrecta, por lo que el contrato debe incluir estados de éxito y error.', whyItMattersMarkdown: 'Una clave en JavaScript del navegador puede copiarse y usarse con tu cuenta. Además, confiar ciegamente en la salida de un modelo puede filtrar datos o ejecutar acciones incorrectas. Un contrato acotado reduce costo, riesgo y superficie de ataque.', realWorldContextMarkdown: 'Para SASE, un mentor IA futuro podría recibir la duda del alumno y un fragmento curricular autorizado. No debe recibir secretos, expedientes completos ni la clave de proveedor; el servidor valida la salida antes de mostrar una sugerencia.', narrationText: 'Las claves de LLM viven en el servidor. El cliente envía contexto mínimo y recibe una respuesta validada con estados claros de éxito o error.' },
    examples: [{ id: 'ejemplo-contrato', title: 'Contrato mínimo entre cliente y servidor', code: 'Cliente -> POST /api/mentor { pregunta, contextoPermitido }\nServidor -> usa la clave secreta y valida la respuesta\nServidor -> { estado: "ok", sugerencia: "..." }\n\nLa clave del proveedor nunca aparece en el navegador.', explanationMarkdown: 'El navegador solo conoce el endpoint propio y los campos públicos del contrato.' }],
    guidedPractice: [
      { id: 'guiado-1', order: 1, promptMarkdown: '¿Dónde debe vivir una clave secreta de proveedor LLM?', options: ['En el servidor o función protegida', 'En una variable de React publicada al navegador', 'Dentro del prompt del usuario', 'En localStorage'], correctAnswer: 'En el servidor o función protegida', hints: ['Todo lo que llega al navegador puede inspeccionarse.'], explanationMarkdown: 'El servidor controla la clave; el cliente nunca debe recibirla.' },
      { id: 'guiado-2', order: 2, promptMarkdown: '¿Qué debe hacer el servidor antes de devolver una salida de IA?', options: ['Validar la forma y devolver solo campos permitidos', 'Reenviar todo sin revisar', 'Guardar la clave en localStorage', 'Confiar en que el modelo siempre tiene razón'], correctAnswer: 'Validar la forma y devolver solo campos permitidos', hints: ['Una respuesta externa es entrada no confiable hasta validarla.'], explanationMarkdown: 'Validar contrato y campos reduce resultados inesperados o datos no deseados.' },
      { id: 'guiado-3', order: 3, promptMarkdown: '¿Qué contexto es más apropiado para una petición de mentor?', options: ['La pregunta y el fragmento curricular mínimo relevante', 'Todos los expedientes del grupo', 'La clave del proveedor', 'Todo el historial privado del usuario'], correctAnswer: 'La pregunta y el fragmento curricular mínimo relevante', hints: ['Menos contexto innecesario reduce exposición.'], explanationMarkdown: 'El principio de minimización entrega solo lo necesario para resolver la tarea.' },
    ],
    challenge: { id: 'reto-api-segura', promptMarkdown: 'Completa `normalizarRespuestaMentor` para aceptar solo respuestas con una sugerencia de texto no vacía y devolver un error controlado en los demás casos.', starterCode: 'function normalizarRespuestaMentor(respuesta) {\n  // Si respuesta.estado === "ok" y respuesta.sugerencia es texto no vacío:\n  // retorna "Sugerencia: <sugerencia>"\n  // En otro caso retorna "Error controlado: respuesta no válida"\n  return "";\n}\n\nconsole.log(normalizarRespuestaMentor({ estado: "ok", sugerencia: "Prueba el caso límite de 6." }));\nconsole.log(normalizarRespuestaMentor({ estado: "ok", sugerencia: "" }));', language: 'javascript', timeoutMs: 2000, checks: [{ id: 'check-ok', type: 'stdoutIncludes', label: 'Respuesta válida normalizada', value: 'Sugerencia: Prueba el caso límite de 6.', failureMessage: 'Una respuesta ok con sugerencia no vacía debe mostrarse.' }, { id: 'check-invalida', type: 'stdoutIncludes', label: 'Respuesta inválida controlada', value: 'Error controlado: respuesta no válida', failureMessage: 'Una sugerencia vacía debe rechazarse con un error controlado.' }], hints: ['Revisa estado, typeof sugerencia y sugerencia.trim().length.', 'Usa return temprano para el caso válido.'], expectedEvidenceMarkdown: 'La consola debe mostrar una sugerencia válida y luego el error controlado para la respuesta incompleta.' },
    reflectionPromptMarkdown: 'Si crearas un mentor IA para una lección, ¿qué campos mínimos enviaría el cliente y qué salida validarías antes de mostrarla?', masteryCriteria: ['Sé que las claves de proveedor no llegan al navegador.', 'Minimizo contexto y datos enviados al LLM.', 'Valido una respuesta antes de mostrarla.', 'Escribí una normalización con éxito y error controlado.']
  },
];
