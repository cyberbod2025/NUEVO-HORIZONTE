...
- **v2** (`src/types/lesson.ts` `LessonV2`, `src/data/lessonsV2.ts` `LESSONS_V2`): the current content contract — multiple learning objectives, worked examples, a multi-step `guidedPractice` array (each step with progressive `hints`), a `challenge` with several declarative `checks`, a reflection prompt, and explicit `masteryCriteria`. 21 pilot lessons exist so far (legacy modules 1–7: fundamentals, modern JS/async, Git/Conventional Commits, TypeScript, React props/useState, HTTP/Express/CRUD, SQL/relations/Supabase RLS), shown in `TimelineView` under a separate "Piloto v2" section with its own sequential unlock (still using `isModuleUnlocked`). Modules 8–12 are not migrated yet.
