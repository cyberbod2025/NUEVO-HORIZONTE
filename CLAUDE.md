...
- **v2** (`src/types/lesson.ts` `LessonV2`, `src/data/lessonsV2.ts` `LESSONS_V2`): the current content contract — multiple learning objectives, worked examples, a multi-step `guidedPractice` array (each step with progressive `hints`), a `challenge` with several declarative `checks`, a reflection prompt, and explicit `masteryCriteria`. 27 pilot lessons exist so far (legacy modules 1–9: fundamentals, modern JS/async, Git/Conventional Commits, TypeScript, React props/useState, HTTP/Express/CRUD, SQL/relations/Supabase RLS, automated testing, prompts/RAG/LLM API boundaries), shown in `TimelineView` under a separate "Piloto v2" section with its own sequential unlock (still using `isModuleUnlocked`). Modules 10–12 are not migrated yet.
